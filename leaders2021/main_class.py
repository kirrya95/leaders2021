...
class FindLostAnimal():
    ...
    def __save_features(self, features, saved_files, filename, id, address, date):
        ...
        dictionary["filename"] = [filename.split('/')[-1]] * (n + 1)
        ...
        for i in range(n):
            dictionary["cropped_file_name"][i] = saved_files[i].split('/')[-1]
            ...
            dictionary["is_the_owner_there"][i] = str(features[i][3])
        ...
        for box, label in zip(boxes, labels):
            ...
            if label in self.indices_animal_detection:
                ...
                if pred in self.indices_cat_dog_classification:
                    ...
                    name_of_saved_picture = '/'.join(os.path.abspath('app.py').split('/')[:-2]) + "/build/cropped/test" + str(num_start + cnt) + ".jpg"
